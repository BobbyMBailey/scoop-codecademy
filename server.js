...
let createComments = function( url, request) {
  ...
  if (!BaseController.hasValidRequestBody(request, [])) {
    return {
...
let upvoteComments;
let downvoteComments;
let updateComments;
let deleteComments;

let getArticles = function(url, request) {
  return BaseController.getCollectionByKey('articles', url, request);
}

let getComments = function(url, request) {
  return BaseController.getCollectionByKey('comments', url, request);
}

let createArticle = function(url, request) {
  if (!BaseController.isUserAuthenticated(request, database.users)) {
    return {
      body: {'errorMessage': 'User is Not Authenticated / Logged In'},
      status: 401
